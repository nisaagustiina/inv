...
class item_m extends CI_Model {
    ...
    public function get($id=null){
        $this->db->select('p_item.*,p_group.name_group as group, p_category.name_category as category, p_unit.name_unit as unit, vendor.name_vendor as vendor');
        $this->db->from('p_item');
        $this->db->join('p_category','p_category.id_category=p_item.id_category');
        $this->db->join('p_group','p_group.id_group=p_item.id_group');
        $this->db->join('p_unit','p_unit.id_unit=p_item.id_unit');
        $this->db->join('vendor','vendor.id_vendor=p_item.id_vendor');
        if($id != null){
            $this->db->where('id_item',$id);
        }
        $this->db->order_by('code_item','asc');
        $query = $this->db->get();
        return $query;
    }
    ...
    public function update_stock_in($data){
        ...
        $sql = "UPDATE p_item SET stock = stock+'$qty' WHERE id_item = '$id_item'";
        $this->db->query($sql);
    }
